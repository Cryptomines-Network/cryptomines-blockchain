...
class ExecutionClient:
    ...
    def _ensure_web3_init(self) -> None:
        ...
        ec_config = self.full_node.config.get("execution_client")
        selected_network = self.full_node.config.get("selected_network")
    ...
    async def _forkchoice_update(
        ...
        while True:
            ...
            if curr.prev_transaction_block_hash == self.full_node.constants.GENESIS_CHALLENGE:
                final_height = None
                final_hash = BLOCK_HASH_NULL
                break
            
            curr = self.full_node.blockchain.block_record(curr.prev_transaction_block_hash)
    ...
    async def _replay_sync(
        ...
        curr = self.full_node.blockchain.get_peak()
        ...
        while True:
            if record.is_transaction_block:
                ...
                block = await self.full_node.blockchain.get_full_block(record.header_hash)
            ...
            h += 1
            record = self.full_node.blockchain.height_to_block_record(h)
